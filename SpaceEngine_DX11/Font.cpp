...
void Font::BuildVertexArray(void* vertices, char* sentence, float drawX, float drawY)
{
	...
	for (i = 0; i < numLetters; i++)
	{
		...
		else
		{
			...
			vertexPtr[index].position = XMFLOAT3((drawX + m_Font[letter].size), (drawY - 13), 0.0f);  // Bottom right.
			vertexPtr[index].texture = XMFLOAT2(m_Font[letter].right, 1.0f);
			index++;

			vertexPtr[index].position = XMFLOAT3(drawX, (drawY - 13), 0.0f);  // Bottom left.
