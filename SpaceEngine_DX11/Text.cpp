...
bool Text::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight, XMFLOAT4X4 baseViewMatrix)
{
	bool result;
...
bool Text::Render(ID3D11DeviceContext* deviceContext, ShaderFont* FontShader, XMFLOAT4X4 worldMatrix, XMFLOAT4X4 orthoMatrix)
{
	bool result;
...
bool Text::RenderSentence(SentenceType* sentence, ID3D11DeviceContext* deviceContext, ShaderFont* FontShader, XMFLOAT4X4 worldMatrix, XMFLOAT4X4 orthoMatrix)
{
...
bool Text::SetRenderCount(int count, ID3D11DeviceContext* deviceContext)
{
	...
	// Setup the cpu string.
	strcpy_s(renderString, "Polygon Count: ");
	strcat_s(renderString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, renderString, 10, 150, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
