...
bool Image::Initialize(ID3D11Device* device, ID3D11DeviceContext* context, int screenWidth, int screenHeight, WCHAR* textureFilename, int imageWidth, int imageHeight)
{
	bool result;
...
bool Image::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	// If the position we are rendering this image to has not changed then don't update the vertex buffer since it
...
bool Image::LoadTexture(ID3D11Device* device, ID3D11DeviceContext* context, WCHAR* filename)
{
	bool result;
